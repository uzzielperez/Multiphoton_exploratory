...
triSortedPhotons::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  const pat::Photon *photon_1 = NULL;
  const pat::Photon *photon_2 = NULL;
  const pat::Photon *photon_3 = NULL; 
 ...
 if(photons->size()>2) cout << photon_1 ->pt() << " " << photon_2 ->pt() << " " << photon_3 ->pt() << endl; 
 ...
 cout <<"SIZE: "<< phosize  << "; lead_pt: " << pholead1 << "; sub_leading_photon: " << pholead2 << "; sub_sub_leading: " << pholead3 << endl;
  ...
  //Fill struct info
   //PHOTON1:
   photon1Info.pt = photon_1->pt(); 
   photon1Info.eta = photon_1->eta();
   photon1Info.phi = photon_1->phi();
   photon1Info.sceta = photon_1->superCluster()->eta();
   photon1Info.scphi = photon_1->superCluster()->phi();
   //PHOTON2:
   photon2Info.pt = photon_2->pt(); 
   photon2Info.eta = photon_2->eta();
   photon2Info.phi = photon_2->phi();
   photon2Info.sceta = photon_2->superCluster()->eta();
   photon2Info.scphi = photon_2->superCluster()->phi(); 
   //PHOTON3:
   photon3Info.pt = photon_3->pt(); 
   photon3Info.eta = photon_3->eta();
   photon3Info.phi = photon_3->phi();
   photon3Info.sceta = photon_3->superCluster()->eta();
   photon3Info.scphi = photon_3->superCluster()->phi();
